...
def test_paired_ive_calculation():
    ...
    paired_df = compute_paired_ive(raw_df, output_path="tests/temp_paired.csv")
...
def test_cross_lingual_contrasts():
    raw_df = create_synthetic_judgments_for_testing()
    paired_df = compute_paired_ive(raw_df, output_path="tests/temp_paired.csv")
    contrasts_df = compute_cross_language_contrasts(paired_df, output_path="tests/temp_contrasts.csv")
...
def test_scenario_clustered_bootstrap():
    raw_df = create_synthetic_judgments_for_testing()
    paired_df = compute_paired_ive(raw_df, output_path="tests/temp_paired.csv")
    boot_df = scenario_clustered_bootstrap(paired_df, n_resamples=500, seed=42)
